...
const eventfulKey = "SFdqgfDLvpk62fwL";
...
firstImage.getFirstImageURL("cavan")
.then((a) => {
	console.log(a);
});
	...
	return new Promise((resolve, reject) => {
		rp(path, function (error, response, body) {
			...
			body.events.event.map((e) => {
				...
				firstImage.getFirstImageURL(e.title).then((b) => {
					event = {};
					event.name = e.title;
					event.imageUrl = e.image ? e.image.medium.url : b;
					...
					console.log("out of", body.events.event.length);
